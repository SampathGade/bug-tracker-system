...
const BugsBoard = ({ filters, onEditBug, sprint }) => {
  const [bugs, setBugs] = useState([]);
  const [showStats, setShowStats] = useState(false); // Fetch current sprint, defaulting to 'Backlog' if not set
  ...
  useEffect(() => {
    ...
    fetchBugs();
  }, [filters, sprint]); // Add currentSprint to the dependency array
  ...
  const handleDragEnd = async (bugId, newStatus) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/bug/updateBugStatus`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
